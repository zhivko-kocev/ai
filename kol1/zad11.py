...
class Maze(Problem):
    ...
    def color(self, state, which):
        new_state = [list(obj) for obj in state]
        if new_state[which][2] == 0:
            new_state[which][2] = 1
        else:
            new_state[which][2] = 0
        # gore
        if which - self.size >= 0:
            if new_state[which - self.size][2] == 0:
                new_state[which - self.size][2] = 1
            else:
                new_state[which - self.size][2] = 0

        # dolu
        if which + self.size < (self.size * self.size):
            if new_state[which + self.size][2] == 0:
                new_state[which + self.size][2] = 1
            else:
                new_state[which + self.size][2] = 0
        ...
        # levo
        if which % self.size != 0:
            if new_state[which - 1][2] == 0:
                new_state[which - 1][2] = 1
            else:
                new_state[which - 1][2] = 0
        # desno
        if (which + 1) % self.size != 0:
            if new_state[which + 1][2] == 0:
                new_state[which + 1][2] = 1
            else:
                new_state[which + 1][2] = 0

        new_state = tuple(tuple(obj) for obj in new_state)
        return new_state

    def successor(self, state):
        neighbors = {}
        for i in range(self.size * self.size):
            new_state = self.color(state, i)
            if new_state is not None:
                neighbors[f'x: {state[i][0]}, y: {state[i][1]}'] = new_state
    ...
    def goal_test(self, state):
        colors = [color[2] for color in state]
        for color in colors:
            if color == 0:
                return False
        return True
