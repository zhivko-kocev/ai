...
class GhostOnSkates(Problem):
    ...
    def check_valid(self, state):
        return state not in self.walls and (0 <= state[0] <= self.n) and (0 <= state[1] <= self.n)

    def move(self, state, direction, increment):
        state = list(state)
        state[direction] += increment
        state = tuple(state)
        if not self.check_valid(state):
            return None
        return state
    ...
    def successor(self, state):
        ...
        u1 = self.move(state, 1, 1)
        if u1 is not None:
            successors["Gore 1"] = u1
        u2 = self.move(state, 1, 2)
        if u2 is not None:
            successors["Gore 2"] = u2
        u3 = self.move(state, 1, 3)
        if u3 is not None:
            successors["Gore 3"] = u3
        r1 = self.move(state, 0, 1)
        if r1 is not None:
            successors["Desno 1"] = r1
        r2 = self.move(state, 0, 2)
        if r2 is not None:
            successors["Desno 2"] = r2
        r3 = self.move(state, 0, 3)
        if r3 is not None:
            successors["Desno 3"] = r3
    ...
    def h(self, node):
        state = node.state
        goal = self.goal
        return (abs(goal[0] - state[0]) + abs(goal[1] - state[1])) / 3
