...
class Snake(Problem):
    ...
    def h(self, node):
        if len(node.state[2]) == 0:
            return 0
        m = self.manhattan_distance(node.state[0][0], node.state[2][0])
        for apple in node.state[2]:
            m = min(m, self.manhattan_distance(apple, node.state[0][0]))
        return m
...
if __name__ == "__main__":
    ...
    snake = Snake((((0, 7), (0, 8), (0, 9)), 'S', tuple(zeleni_jabolki)))
