...
class Ball(Problem):
    ...
    def __init__(self, initial, board_size, walls, goal=None):
        super().__init__(initial, goal)
        self.board_size = board_size
        self.walls = walls

    def check_valid(self, state, which, ball):

        return ball in state and tuple(state[which]) not in self.walls and (
                0 <= state[which][0] < self.board_size) and (0 <= state[which][1] < self.board_size)

    def move(self, state, which, direction):
        state = [list(item) for item in state]
        ball = state[which].copy()
        if direction == "Gore Levo":
            state[which][1] += 2
            state[which][0] -= 2
            ball[1] += 1
            ball[0] -= 1
        if direction == "Gore Desno":
            state[which][1] += 2
            state[which][0] += 2
            ball[1] += 1
            ball[0] += 1
        if direction == "Dolu Levo":
            state[which][1] -= 2
            state[which][0] -= 2
            ball[1] -= 1
            ball[0] -= 1
        if direction == "Dolu Desno":
            state[which][1] -= 2
            state[which][0] += 2
            ball[1] -= 1
            ball[0] += 1
        ...
        if direction == "Desno":
            state[which][0] += 2
            ball[0] += 1
        if not self.check_valid(state, which, ball):
            return None

        state.remove(ball)
        new_state = tuple([tuple(obj) for obj in state])
        return new_state

    def successor(self, state):
        neighbors = {}
        directions = ("Gore Levo", "Gore Desno", "Dolu Levo", "Dolu Desno", "Levo", "Desno")
        for i in range(len(state)):
            for direction in directions:
                new_state = self.move(state, i, direction)
                if new_state is not None:
                    neighbors[f'{direction}: (x={state[i][0]},y={state[i][1]})'] = new_state
...
if __name__ == "__main__":
    ...
    balls = tuple(balls)
    walls = tuple(walls)
    maze = Ball(balls, board_size, walls, ((board_size-1)//2, board_size - 1))
    sol = breadth_first_graph_search(maze)
    if sol is not None:
        print(sol.solution())
