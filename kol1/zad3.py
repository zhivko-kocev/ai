...
class Problem:
    def __init__(self, initial, length, goal=None):
        self.initial = initial
        self.goal = goal
        self.length = length

    def valid(self, state, frm, to):
        f = len(state[frm]) != 0
        s = len(state[to]) != self.length
        if len(state[to]) != 0 and f and s:
            t = state[frm][-1] < state[to][-1]
        else:
            t = True
        return f and s and t

    def move(self, state, frm, to):
        bars = tuple(list(bar) for bar in state)
        if not self.valid(bars, frm, to):
            return None
        bars[to].append(bars[frm].pop())
        return tuple(tuple(bar) for bar in bars)
            ...
            for j in range(len(state)):
                if i != j:
                    ...
                    if new_state is not None:
                        neighbors[f'MOVE TOP BLOCK FROM PILLAR {i + 1} TO PILLAR {j + 1}'] = new_state
        return neighbors
...
if __name__ == "__main__":
    ...
    start[start_index] = tuple(map(int, start[start_index].split(",")))
    for i in range(len(start)):
        if i != start_index:
            start[i] = tuple()
    start = tuple(start)
    ...
    end[end_index] = tuple(map(int, end[end_index].split(",")))
    for i in range(len(start)):
        if i != end_index:
            end[i] = tuple()
    end = tuple(end)

    prob = Problem(start,len(start[start_index]),end)
    sol = breadth_first_graph_search(prob)
    if sol is not None:
