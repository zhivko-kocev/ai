...
class Football(Problem):
    ...
    def check_valid(self, state):
        ...
        return ball not in self.invalid_ball_positions and (0 <= ball[0] <= 7 and 0 <= ball[1] <= 5) \
            and (0 <= player[0] <= 7 and 0 <= player[
                1] <= 5) and ball not in self.players and player not in self.players
    ...
    def h(self, node):
        first = abs(self.goal[0][0] - node.state[1][0]) + abs(self.goal[0][1] - node.state[1][1])
        second = abs(self.goal[1][0] - node.state[1][0]) + abs(self.goal[1][1] - node.state[1][1])
