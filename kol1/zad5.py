...
class Squares(Problem):
    ...
    def move(self, state, which, direction):
        new_state = tuple(list(obj) for obj in state)
        if direction == "levo":
            new_state[which][0] -= 1
        if direction == "desno":
            new_state[which][0] += 1
        ...
        if not self.check_valid(new_state):
            return None
    ...
    def successor(self, state):
        ...
        for i in range(1, 6):
            for move in moves:
                new_state = self.move(state, i - 1, move)
                if new_state is not None:
                    succ[f'Pomesti kvadratche {i} {move}'] = new_state
